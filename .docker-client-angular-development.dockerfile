...
CMD /bin/bash -c "npm rebuild && envsubst < ./src/environments/environment-template.ts > ./src/environments/environment.ts && ng serve --disable-host-check --hmr=true --poll=1000 --host 0.0.0.0"
